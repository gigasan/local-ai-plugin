    ...
    fun append(event: StreamEvent) {
        ...
        when (event.type) {
            ...
            EventType.CHAT_START -> {
                ...
                logger.info("AIResultBuilder: CHAT_START $model")
                event.indicatorText = String.format("CHAT_START %s", model)
            }

            EventType.MODEL_LOAD_START -> {
                progress = 0.0
                logger.info("AIResultBuilder: MODEL_LOAD_START event.content=${event.content}")
            ...
            EventType.MODEL_LOAD_PROGRESS -> {
                ...
                logger.info("AIResultBuilder: MODEL_LOAD_PROGRESS $progress")
                event.indicatorText = String.format("MODEL_LOAD_PROGRESS %s %.2f%%", model, progress * 100.0)
            }

            EventType.MODEL_LOAD_END -> {
                logger.info("AIResultBuilder: MODEL_LOAD_END event.content=${event.content}")
                event.indicatorText = String.format("MODEL_LOAD_PROGRESS %s %.2f%%", model, progress * 100.0)
            }
            ...
            EventType.MESSAGE_DELTA -> {
                output.append((event.payload as? StreamPayload.Message)?.content?: "")
            ...
            EventType.CHAT_END -> {
                ...
                logger.info("AIResultBuilder: CHAT_END result=${result}")
                if (result != null) {
