...
import com.gigasan.ai.config.BackendEndpoints
...
class Client(private val project: Project) {
    ...
    fun send(ctx: ChatContext): Result {
        ...
        val (request, parser) = when (backend.api to endpoint) {
            BackendApi.LM_STUDIO_API to "/api/v1/chat" -> LmStudioAdapter.toRequest(project, ctx, baseUrl+endpoint, apiKey) to AIResponseParser::parse
            BackendApi.OPEN_AI_API to "/v1/responses" -> ResponsesAdapter.toRequest(project, ctx, baseUrl+endpoint, apiKey) to AIResponseParser::parse
            BackendApi.OPEN_AI_API to "/v1/chat/completions" -> ChatCompletionsAdapter.toRequest(project, ctx, baseUrl+endpoint, apiKey) to AIResponseParser::parse
            else -> DefaultAdapter.toRequest(project, ctx, baseUrl+endpoint, apiKey) to AIResponseParser::parse
        }
