    ...
    fun analyzeClassDetailed(ktClass: KtClass): String {
        ...
        analyze(ktClass) {
            ...
            classSymbol.memberScope.callables
                ...
                .forEach { prop ->
                    sb.append("  * Property: ${prop.name}: ${prop.returnType.render(
                        renderer = kaTypeRenderer,
                        position = Variance.INVARIANT
                    )}\n\n")
                }
...
/*
fun handleAiCommand(jsonCommand: String, project: Project): String {
    // Представим, что мы распарсили JSON в объект Command
    // val command = parse(jsonCommand)

    return when (command.type) {
        "ANALYZE_FILE" -> {
            val virtualFile = project.guessProjectDir()?.findFileByRelativePath(command.path)
            val psiFile = virtualFile?.let { PsiManager.getInstance(project).findFile(it) }
            psiFile?.let { ProjectAnalyzer().getFileStructure(it) } ?: "File not found"
        }
        "READ_FILE" -> {
            // Чтение полного текста файла для глубокого анализа
            readFileContent(command.path, project)
        }
